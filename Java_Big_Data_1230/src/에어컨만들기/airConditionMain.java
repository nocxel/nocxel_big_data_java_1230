...
public class airConditionMain {
    public static void main(String[] args) throws InterruptedException {
        ...
        boolean isSetTemp = false; // 온도가 변경되어야할 조건
        ...
        if (isON.equalsIgnoreCase("yes")) {
            ...
            while(true) {
                ...
                if (isSetTemp) {
                    if (lgAirCon.isHeater()) lgAirCon.setCurrTemp(1); // 히터 켜져있으면 에어컨 현재온도 1
                    if (lgAirCon.isCooler()) lgAirCon.setCurrTemp(-1); // 쿨러 켜져있으면 에어컨 현재온도 -1
                    lgAirCon.airConInfo();
                    isSetTemp = false;
                    elapsedTime = 0;
                }
                ...
                if(lgAirCon.getCurrTemp() == lgAirCon.getSetTemp()) { // 현재온도가 설정온도와 같아지면
