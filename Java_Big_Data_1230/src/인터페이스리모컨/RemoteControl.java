...
// 모든 메소드는 자동으로 추상메소드가 됩니다. (public abstract)
// 디폴트 메소드 : 예외적으로 구현부를 가질 수 있으며 상속 받은 클래스에서 재정의 가능 합니다.
// 정적메소드를 포함 할 수 있습니다. (정적메소드는 객체와 상관없이 인터페이스 타입으로 사용 됩니다.)

public interface RemoteControl{
    int MAX_VOLUME = 100; // 상수로 정의 하지 않았지만 정적인 상수이다. (인터페이스 생성 시 만들어지고 객체는 안됨)
    int MIN_VOLUME = 0; // 자동으로 final static
    void turnOn(); // 자동으로 public abstract 붙는다 why? 인터페이스는 기본적으로 모든 메소드가 추상메소드이다.
    void turnOff();
    ...
    void getInfo();
    // 자바 버전 1.8 이후에 추가 되었으며 예외적으로 메소드의 구현를 가질 수 있습니다.
