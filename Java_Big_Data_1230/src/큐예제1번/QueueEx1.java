...
// Queue : FIFO 구조, LinkedList구조를 사용하고 있으며, LinkedList 기능 중 큐 관련 기능으로 제한 됨
// add(e) : 맨 뒤에 요소를 삽입 - 예외구문이 발생함 (스택이 꽉 찼다던가)
...
public class QueueEx1 {
    public static void main(String[] args) {
        ...
        msgQ.add(new Message("SMS", "채원"));
        msgQ.offer(new Message("KaKao", "카리나"));
        while (!msgQ.isEmpty()) {
            Message msg = msgQ.poll(); // 큐에서 한 개의 메시지를 꺼냄
            switch (msg.command) {
                case "Mail":
                    System.out.println(msg.to + "에게 메시지를 보냅니다.");
                    break;
                case "SMS":
